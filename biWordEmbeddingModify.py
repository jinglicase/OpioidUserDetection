...
def getResult(max_length, embeddingDim, t, X_train, X_test, Y_train, Y_test, embedding_matrix, method, flagExistEmbedding=False, flagTrainable=False):
    ...
    return tn, fp, fn, tp, impWords_dict,train_loss_list,val_loss_list
...
def testOneFold(max_length, embeddingDim, t, padded_docs, Y, embedding_matrix, method, flagExistEmbedding = False, flagTrainable = False):
    ...
    kfold = StratifiedKFold(n_splits=num_splits, shuffle=True, random_state=1)
    tnSum = 0
    fpSum = 0
    fnSum = 0
    tpSum = 0
    ...
    for i in range(numIter):
        ...
        for train, test in kfold.split(padded_docs, Y):
            # print("train",padded_docs[train])
            # print("y",Y[train])
            # print("embedding matrix",embedding_matrix)

            tn, fp, fn, tp, impWords_dict,train_loss_list,val_loss_list = getResult(max_length, embeddingDim, t, padded_docs[train], padded_docs[test], Y[train], Y[test], embedding_matrix,
                                                    method, flagExistEmbedding, flagTrainable)
            tnSum += tn
            fpSum += fp
            fnSum += fn
            tpSum += tp
            train_loss_listSUM += train_loss_list
            val_loss_listSUM += val_loss_list
            # val_loss_listSUM = [val_loss_listSUM[i]+val_loss_list[i] for i in range(min(len(val_loss_listSUM),len(val_loss_list)))]
    ...
    # train_loss_listSUM = train_loss_listSUM/15
    # val_loss_listSUM = val_loss_listSUM/15
    ...
    print("Separate F1", separate_f1)
    print("F1 AVE", sum(separate_f1)/15)
    # print('acc',acc)
    print("train loss",train_loss_listSUM)
    print("val loss",val_loss_listSUM)
...
def main():
    ...
    variantName = "variant3" #choose the variant type
    print("Fine 2")
